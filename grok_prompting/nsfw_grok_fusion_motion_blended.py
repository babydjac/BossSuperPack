...
class NSFWGrokFusionPro:
    ...
    def INPUT_TYPES(cls):
        return {
            "required": {
                ...
                "motion_text": ("STRING", {"multiline": True, "default": "slow pan, smooth movement"}),
    ...
    def grok3_generate_prompts(self, text_description, motion_text, key):
        headers = {"Authorization": f"Bearer {key}", "Content-Type": "application/json"}
        motion_instruct = f"Integrate the following physical motion into the video description naturally: {motion_text}"
        user_prompt = f"{text_description}\n\n{motion_instruct}"
        payload = {
            "model": "grok-3-latest",
            "messages": [
                {
    ...
    def process(self, image, api_key, prepend_text, append_text, motion_text, score_strategy, token_limit, debug_output):
        try:
            ...
            prompt_response = self.grok3_generate_prompts(description, motion_text, key)
            ...
            if debug_output == "enable":
                debug = (
                    f"Score Tags: {score_tags}\n"
                    f"Prompt: {ponyxl_prompt}\n"
                    f"Final: {trimmed_prompt}\n"
                    f"Motion Injected: {motion_text}\n"
                    f"WAN Prompt: {wan_prompt}"
                )
